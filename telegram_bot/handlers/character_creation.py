...
async def set_race(message: types.Message, state=FSMContext):
    ...
    await FSMCharacter.next()
    await message.reply(f'{data["race"]} - отличный выбор! Теперь расскажи, какой класс ты выбрал для своих странствий', reply_markup=cancel_menu)
...
async def set_clas(message: types.Message, state=FSMContext):
    ...
    await FSMCharacter.next()
    await message.reply(
        f'Хмм, {data["clas"]}... У него есть достойная история? Поведай ее или же выбери одно из предложенных сказаний', reply_markup=cancel_menu)
...
async def set_origin(message: types.Message, state=FSMContext):
    ...
    await FSMCharacter.next()
    await message.reply(f'История от {data["origin"]} я еще не слыхал. Теперь скажи, какого уровня ты смог достичь?', reply_markup=cancel_menu)
...
async def stop_creating_character(call: types.CallbackQuery, state=FSMContext):
    await state.finish()
    await call.message.edit_reply_markup(reply_markup=None)
    await call.message.answer("Видимо сейчас ты не готов поделиться своей историей... Не переживай, я всегда буду здесь, чтобы"
